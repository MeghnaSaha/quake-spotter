...
public class QueryUtils {
    ...
    public static List<Earthquake> extractFeatureFromJson(String earthquakeJson) {
        ...
        try {
            JSONArray response = new JSONObject(earthquakeJson).optJSONArray("features");
            for (int i = 0; i < response.length(); i++){
                earthquakes.add(new Earthquake(
                        response.getJSONObject(i).optJSONObject("properties").optDouble("mag"),
                        response.getJSONObject(i).optJSONObject("properties").optString("place"),
                        response.getJSONObject(i).optJSONObject("properties").optLong("time"),
                        response.getJSONObject(i).optJSONObject("properties").optString("url")
                ));
            }
